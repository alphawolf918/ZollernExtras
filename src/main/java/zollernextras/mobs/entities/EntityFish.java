...
public class EntityFish extends EntityWaterMob implements IMob, IAnimals {
	 ...
	 * boolean flag = itemstack != null && itemstack == new
	 * ItemStack(Item.getItemById(383)); if (flag) { return
	 * super.interact(par1EntityPlayer); } if (itemstack == Items.fishFlakes ||
	 * itemstack == Items.fishFood) { this.setFishSize(this.getFishSize() +
	 * 0.1F); float maxHealth = this.getMaxHealth(); IAttributeInstance
	 * attrMaxHealth = this
	 * .getEntityAttribute(SharedMonsterAttributes.maxHealth); if
	 * (attrMaxHealth.getAttributeValue() <= this.maxHealthLimit) { maxHealth +=
	 * 2.0F; this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
	 * .setAttribute(maxHealth); this.setHealth(maxHealth); } if
	 * (!par1EntityPlayer.capabilities.isCreativeMode) { if (itemstack ==
	 * Items.fishFlakes) { --itemstack.stackSize; } else if (itemstack ==
	 * Items.fishFood) { itemstack.damageItem(1, par1EntityPlayer); } if
	 * (itemstack.stackSize <= 0) {
	 * par1EntityPlayer.inventory.setInventorySlotContents(
	 * par1EntityPlayer.inventory.currentItem, (ItemStack) null); } } return
	 * true; } return false; }
	 */
